var AdminGlobal = function () {
    ...
    return {
        init: function () {
            ...
            $(".page-container > .page-content").css({"min-height": c + "px"});
            ...
            if (b.size() > 0) {
                    ...
                    if (l.value == "/dashboard") {
                        ...
                        if (l.value == "/") {
                            ...
                            if (l.value == "/lock") {
                                ...
                                $("body").ajaxPostURL({url: WEB_ROOT + "/layout!lock", confirmMsg: false})
                            } else {
                ...
                $('.sidebar-search input[name="search"]').autocomplete({
                    autoFocus: true, source: function (k, i) {
                        ...
                        return i(j.map(function () {
                            ...
                            if (m.indexOf(o) > -1 || n.indexOf(o) > -1) {
                                return {label: $.trim(n), link: l, href: l.attr("href")}
                            }
                        }))
                    }, minLength: 1, select: function (j, k) {
                        var i = k.item;
            ...
            var e = $("#header_notification_bar");
            if (e.size() > 0) {
                $.ajax({
                    dataType: "json", method: "get", url: WEB_ROOT + "/notify-message/count", success: function (j) {
                        var k = j.data;
                        if (k > 0) {
                            var l = "<a href='javascripts:;' rel='address:/admin/profile/notify-message?readed=no|未读公告信息列表'>您有 " + k + " 条未读公告信息</a>";
                            $(".message-info", e).html(l);
                            $(".badge", e).html(k).show()
                        } else {
                            var l = "<a href='javascripts:;' rel='address:/admin/profile/notify-message|公告信息列表'>暂无未读，点击查看公告信息列表</a>";
                            $(".message-info", e).html(l);
                            $(".badge", e).html(0).hide()
                        }
                    }
            ...
            var g = $("#header_inbox_bar");
            if (g.size() > 0) {
                $.ajax({
                    dataType: "json", method: "get", url: WEB_ROOT + "/user-message/count", success: function (j) {
                        var k = j.data;
                        if (k > 0) {
                            var l = "<a href='javascripts:;' rel='address:/admin/profile/user-message?readed=no|未读个人消息列表'>您有 " + k + " 条未读个人消息</a>";
                            $(".message-info", g).html(l);
                            $(".badge", g).html(k).show()
                        } else {
                            var l = "<a href='javascripts:;' rel='address:/admin/profile/user-message|个人消息列表'>暂无未读，点击查看个人消息列表</a>";
                            $(".message-info", g).html(l);
                            $(".badge", g).html(0).hide()
                        }
                    }
                ...
                if (l.attr("data-url")) {
                    ...
                    if (jQuery().jqGrid) {
                        k.find("table.ui-jqgrid-btable").each(function () {
                            ...
                            n.jqGrid("setGridParam", {datatype: "json", url: o}).trigger("reloadGrid")
                        })
                    }
                }
            });
            ...
            if ($.fn.fileupload) {
                ...
                jQuery("body").on("click", "a.btn-fileinput-trigger", function (j) {
                    ...
                    if (i) {
                        f.find("input[name='attachmentName']").val("_attachment_" + i)
                    }
