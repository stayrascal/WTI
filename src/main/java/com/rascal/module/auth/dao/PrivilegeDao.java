...
public interface PrivilegeDao extends BaseDao<Privilege, Long> {
    ...
    @Query(value = "from Privilege order by code asc")
     ...
     */
    @Query(value = "select distinct p from Privilege p, RoleR2Privilege r2p, Role r where p = r2p.privilege " +
            "and r2p.role = r and r.code in (:roleCodes) and r.disabled = false and p.disabled = false order by p.code asc")
