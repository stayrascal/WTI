            ...
            $("form.form-validation", $container).each(function () {
                ...
                $form.validate({
                    ...
                    submitHandler: function (form) {
                        ...
                        $form.ajaxSubmit({
                            dataType: "json", method: "post", data: postData, success: function (response) {
                                App.unblockUI($form);
                                ...
                                if (response.type == "success" || response.type == "warning") {
                                    ...
                                    if (response.redirect) {
                                        if (response.message) {
                                            ...
                                            var tips = "<div class='alert alert-info'><p>" + response.message + "</p><p>稍等，准备转向...</p></div>";
                                            if ($formActions.size() > 0) {
                                                $formActions.before(tips)
                                            } else {
                                                $form.prepend(tips)
                                            }
                                            setTimeout(function () {
                                    ...
                                    if (dismiss) {
                                        ...
                                        if (ajaxifyReload && $(ajaxifyReload, $modal).size() == 0) {
                                        ...
                                        var reload = submitButton.attr("data-reload");
                                        if (reload != "false") {
                                            var $container = submitButton.closest(".ajax-get-container");
                                            if ($container.size() > 0) {
                                                var url = $container.attr("data-url");
                                                var $navTabs = $container.find(".nav-tabs:first");
                                                if ($navTabs.size() > 0) {
                                                    var active = $navTabs.attr("data-active");
                                                    if (active) {
                                                        url = Util.AddOrReplaceUrlParameter(url, "_tab_active", active)
                                                    }
                                                }
                                                if (response.data && response.data.id) {
                                                    url = Util.AddOrReplaceUrlParameter(url, "id", response.data.id)
                                                }
                                    ...
                                    if (response.type == "failure" || response.type == "error") {
                                        var ret = $form.triggerHandler("form-validate-failure");
                                        if (ret == undefined || ret == true) {
                                            $actions.before("<div class='alert alert-danger alert-validation'>" + response.message + "</div>");
                                            $("img.captcha-img", $form).click()
                                        }
                                    } else {
                                        $actions.before("<div class='alert alert-danger alert-validation'>表单处理异常，请联系管理员</div>");
                                        $("img.captcha-img", $form).click()
                                    }
                                }
