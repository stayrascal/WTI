        ...
        bindBtnSendSmsCode: function () {
            $("body").on("click", ".btn-send-sms-code", function () {
                ...
                if ($mobile.valid()) {
                    ...
                    $.ajax({
                        url: WEB_ROOT + "/w/send-sms-code/" + $mobile.val(),
                        dataType: "json",
                        method: "get",
            ...
            $("body").on("click", ".header a[data-nav-id]", function () {
                var $a = $(this);
                $.cookie("data-nav-id", $a.attr("data-nav-id"), {path: "/"})
            });
            ...
            var infiniteScrolHandler = function () {
                if ($(this).scrollTop() + $(window).height() + 20 >= $(document).height()) {
                    $("table.table-infinite-scroll").each(function () {
                        var $table = $(this);
            ...
            $("input[name='fileUpload']", $singleFileUploadForm).change(function () {
                ...
                $singleFileUploadForm.ajaxSubmit({
                    dataType: "json", method: "post", success: function (response) {
                        if (response.type == "success") {
                            $singleFileUploadTrigger.val(response.data);
                            App.unblockUI($b)
                        }
